            ...
            <div className='z-[4] px-5 flex fixed top-0 h-[85px] w-screen bg-[#0f172a] items-center justify-between'>
                ...
                <div className='flex  items-center space-x-2 mr-2'>
                    <motion.div variants={container}
                        initial="hidden"
                        animate="show" className='text-[#94a9c9] navigation hidden md:flex space-x-6 mx-6 font-mono font-bold'>
                        {links.map((link) => {
                            return <Link variants={item} key={link.key} className={`hover:text-[#1cc2e7] text-base transition-colors duration-300 `} to={link.link}><span className='text-[#1cc2e7] mx-1 '>{link.key}</span>{link.name}</Link>
